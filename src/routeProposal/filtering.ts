...
/*
The staLYFE3 pool (STALYFEADDR) is the main stable pool that holds DAI/USDC/USDT and has the staLYFE3 LBPT.
Metastable pools that contain a project token, i.e. TUSD, paired with staLYFE3 LBPT.
USDC connecting pool (USDCCONNECTINGPOOL) is a metastable pool containing USDC and staLYFE3 LBPT.
This setup should enable paths between the new project metastable pools and other liquidity. I.e. TUSD > LYFE, which would look like:
TUSD>[TUSDstaLYFEPool]>staLYFE3>[ConnectingPool]>USDC>[LyfeWeightedPool]>LYFE
*/
export function getPathsUsingStaLyfePool(
    tokenIn: string,
    tokenOut: string,
    ...
    // This will be the USDC/staLYFE Connecting pool used in Polygon
    ...
    // staLyfe LBPT token is the hop token between token and USDC connecting pool
    const hopTokenStaLyfe = config.staLyfe3Pool?.address;

    if (!usdcConnectingPool || !hopTokenStaLyfe) return [];

    // Finds the best metastable Pool with tokenIn/staLyfe3Lbpt or returns null if doesn't exist
    const metastablePoolIdIn = getHighestLiquidityPool(
        tokenIn,
        hopTokenStaLyfe,
        poolsFiltered
    );
    // Finds the best metastable Pool with tokenOut/staLyfe3Lbpt or returns null if doesn't exist
    ...
    if (metastablePoolIdIn && !metastablePoolIdOut) {
        ...
        // i.e. tokenIn>[metaStablePool]>staLYFE>[usdcConnecting]>USDC>[HighLiqPool]>tokenOut

        const metaStablePoolIn = poolsFiltered[metastablePoolIdIn];

        // tokenIn > [metaStablePool] > staLyfe > [UsdcConnectingPool] > USDC
        const staLyfePath = createPath(
            [tokenIn, hopTokenStaLyfe, usdcConnectingPoolInfo.usdc],
        ...
        return [composePaths([staLyfePath, pathEnd])];
    }
    ...
    if (!metastablePoolIdIn && metastablePoolIdOut) {
        ...
        // i.e. i.e. tokenIn>[HighLiqPool]>USDC>[usdcConnecting]>staLYFE>[metaStablePool]>tokenOut
        ...
        // USDC > [UsdcConnectingPool] > staLyfe > [metaStablePool] > tokenOut
        const staLyfePath = createPath(
            [usdcConnectingPoolInfo.usdc, hopTokenStaLyfe, tokenOut],
        ...
        return [composePaths([pathStart, staLyfePath])];
    }

    // If we're here either the path doesn't use metastable pools (and so will not be routed through StaLYFE)
    // or both input and output tokens are in metastable pools and so should be handled by existing multihop algorithm
    // (because it is tokenIn>[metaStablePoolIn]>staLyfe>[metaStablePoolOut]>tokenOut)
    //
