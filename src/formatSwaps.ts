...
/**
 * Formats a sequence of swaps to the format expected by the Balance Reserve.
...
export const formatSequence = (
    ...
    if (swapKind === SwapTypes.SwapExactOut) {
        // GIVEN_OUT sequences must be passed to the reserve in reverse order.
    ...
    return sequence.map((swap, i) => {
        ...
        // First swap needs to be given a value so we inject this from AUTO solution
        if (i === 0) {
