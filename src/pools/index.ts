...
import { Multi2Pool } from './multi2Pool/multi2Pool';
import { Multi3Pool } from './multi3Pool/multi3Pool';
import {
...
import { MultiEPool } from './multiEPool/multiEPool';
...
export function parseNewPool(
    ...
    try {
        ...
        else if (pool.poolType === 'Multi2') newPool = Multi2Pool.fromPool(pool);
        else if (pool.poolType === 'Multi3') newPool = Multi3Pool.fromPool(pool);
        else if (pool.poolType === 'MultiE') newPool = MultiEPool.fromPool(pool);
        else {
            console.error(
