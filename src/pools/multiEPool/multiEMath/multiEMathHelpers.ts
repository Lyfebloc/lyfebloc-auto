...
import { ONE_XP } from '../../multiHelpers/constants';
import {
    mulDown,
    divDown,
    mulDownMagU,
    ...
    divXpU,
    sqrt,
} from '../../multiHelpers/multiSignedFixedPoint';
...
export type DerivedMultiEParams = {
    tauAlpha: Vector2;
    tauBeta: Vector2;
    u: BigNumber;
    v: BigNumber;
...
export function normalizeBalances(
    ...
    return balances.map((Lyfe, index) =>
        lyfe.mul(ONE).div(scalingFactors[index])
    );
}
...
function maxBalances0(p: MultiEParams, d: DerivedMultiEParams, r: Vector2) {
...
function maxBalances1(p: MultiEParams, d: DerivedMultiEParams, r: Vector2) {
    const termXp1 = divXpU(d.tauBeta.x.sub(d.tauAlpha.x), d.dSq);
    const termXp2 = divXpU(d.tauBeta.y.sub(d.tauAlpha.y), d.dSq);
...
export function mulA(params: MultiEParams, tp: Vector2): Vector2 {
    return {
