...
export type Multi2PoolPairData = PoolPairBase & {
    sqrtAlpha: BigNumber;
    sqrtBeta: BigNumber;
};

export type Multi2PoolToken = Pick<
    SubgraphToken,
    'address' | 'balance' | 'decimals'
>;

export class Multi2Pool implements PoolBase<Multi2PoolPairData> {
    poolType: PoolTypes = PoolTypes.Multi2;
    id: string;
    address: string;
    tokensList: string[];
    tokens: Multi2PoolToken[];
    swapFee: BigNumber;
    totalShares: BigNumber;
    sqrtAlpha: BigNumber;
    sqrtBeta: BigNumber;

    static fromPool(pool: SubgraphPoolBase): Multi2Pool {
        if (!pool.sqrtAlpha || !pool.sqrtBeta)
            throw new Error(
                'Pool missing Multi2 sqrtAlpha and/or sqrtBeta params'
            );

        return new Multi2Pool(
            pool.id,
            pool.address,
            pool.swapFee,
            pool.totalShares,
            pool.tokens as Multi2PoolToken[],
            pool.tokensList,
            pool.sqrtAlpha,
            pool.sqrtBeta
        );
    ...
    parsePoolPairData(tokenIn: string, tokenOut: string): Multi2PoolPairData {
        ...
        const poolPairData: Multi2PoolPairData = {
            id: this.id,
    ...
    getNormalizedLiquidity(poolPairData: Multi2PoolPairData): OldBigNumber {
    ...
    getLimitAmountSwap(
        poolPairData: Multi2PoolPairData,
        swapType: SwapTypes
    ): OldBigNumber {
