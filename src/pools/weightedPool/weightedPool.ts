...
import {
    ...
    _spotPriceAfterSwapExactTokenInForLBPTOut,
    _spotPriceAfterSwapExactLBPTInForTokenOut,
    _spotPriceAfterSwapTokenInForExactLBPTOut,
    _spotPriceAfterSwapLBPTInForExactTokenOut,
    _derivativeSpotPriceAfterSwapExactLBPTInForTokenOut,
    _derivativeSpotPriceAfterSwapExactTokenInForLBPTOut,
...
export class WeightedPool implements PoolBase<WeightedPoolPairData> {
    ...
    _exactTokenInForTokenOut(
        ...
        try {
            if (poolPairData.pairType === PairTypes.TokenToLbpt) {
                returnAmt = _calcLbptOutGivenExactTokensIn(
                    [balanceIn, BigInt(1)],
                ...
                returnAmt = _calcTokenOutGivenExactLbptIn(
                    balanceOut,
                    normalizedWeightOut,
                    amountIn,
                    balanceIn,
    ...
    _tokenInForExactTokenOut(
        ...
        try {
            if (poolPairData.pairType === PairTypes.TokenToLbpt) {
                returnAmt = _calcTokenInGivenExactLbptOut(
                    balanceIn,
                    normalizedWeightIn,
                    amountOut,
                    balanceOut,
                ...
                returnAmt = _calcLbptInGivenExactTokensOut(
                    [balanceOut, BigInt(1)],
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(
        ...
        if (poolPairData.pairType === PairTypes.TokenToLbpt) {
            return _spotPriceAfterSwapExactTokenInForLBPTOut(
                amount,
                poolPairData
            );
        } else if (poolPairData.pairType === PairTypes.LbptToToken) {
            return _spotPriceAfterSwapExactLBPTInForTokenOut(
                amount,
                poolPairData
            );
        } else {
    ...
    _spotPriceAfterSwapTokenInForExactTokenOut(
        ...
        if (poolPairData.pairType === PairTypes.TokenToLbpt) {
            return _spotPriceAfterSwapTokenInForExactLBPTOut(
                amount,
                poolPairData
            );
        } else if (poolPairData.pairType === PairTypes.LbptToToken) {
            return _spotPriceAfterSwapLBPTInForExactTokenOut(
                amount,
                poolPairData
            );
        } else {
    ...
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(
        ...
        if (poolPairData.pairType === PairTypes.TokenToLbpt) {
            return _derivativeSpotPriceAfterSwapExactTokenInForLBPTOut(
                amount,
                poolPairData
            );
        } else if (poolPairData.pairType === PairTypes.LbptToToken) {
            return _derivativeSpotPriceAfterSwapExactLBPTInForTokenOut(
                amount,
                poolPairData
            );
        } else {
