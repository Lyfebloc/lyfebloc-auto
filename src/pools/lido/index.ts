...
Routes[1][`${Lido.USDC[1]}${Lido.wstETH[1]}0`] = {
    ...
    swaps: [
        {
            poolId: Lido.StaticPools.staLyfe[1],
            amount: '',
            assetInIndex: '0',
            assetOutIndex: '1',
            userData: '0x',
...
Routes[42][`${Lido.DAI[42]}${Lido.wstETH[42]}0`] = {
    ...
    swaps: [
        {
            poolId: Lido.StaticPools.staLyfe[42],
            amount: '',
            assetInIndex: '0',
            assetOutIndex: '1',
            userData: '0x',
...
// Uses Reserve queryBatchSwap to get return amount for swap
async function queryBatchSwap(
    ...
    const reserveAddr = '0xBA12222222228d8Ba445958a75a0704d566BF2C8';
    const reserveContract = new Contract(reserveAddr, reserveAbi, provider);
    const funds = {
        sender: AddressZero,
    ...
    try {
        const deltas: BigNumber[] =
            await reserveContract.callStatic.queryBatchSwap(
                swapType,
                swaps,
                assets,
                funds
            );
        // negative amounts represent tokens (or ETH) sent by the Reserve
        if (swapType === SwapTypes.SwapExactIn)
            return deltas[assets.length - 1].mul(-1);
        else return deltas[0];
    } catch (err) {
        console.error(
            `AUTO - Lido Static Route QueryBatchSwap Error. No swaps.`
        );
        return Zero;
    }
}
...
/*
Used when AUTO doesn't support paths with more than one hop.
...
export async function getLidoStaticSwaps(
    ...
    // Unlike main AUTO here we haven't calculated the return amount for swaps so use query call on Reserve to get value.
