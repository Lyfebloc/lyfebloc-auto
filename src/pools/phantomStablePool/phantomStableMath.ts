...
// https://github.com/Lyfebloc/Lyfebloc-core-v2/blob/master/contracts/pools/stable/StableMath.sol
...
amountLBPTOut -> newInvariant -> (amountInProportional, amountInAfterFee) ->
amountInPercentageExcess -> amountIn
*/
export function _tokenInForExactLBPTOut(
    amount: OldBigNumber,
    poolPairData: PhantomStablePoolPairData
): OldBigNumber {
    // The formula below returns some dust (due to rounding errors) but when
    ...
    const bnumBalanceOut = bnum(formatFixed(virtualLbptSupply, decimalsOut));
...
export function _LBPTInForExactTokenOut(
    amount: OldBigNumber,
    poolPairData: PhantomStablePoolPairData
): OldBigNumber {
    // The formula below returns some dust (due to rounding errors) but when
    ...
    // return amountLBPTIn
    const bnumBalanceIn = bnum(formatFixed(virtualLbptSupply, decimalsIn));
...
//////////////////////
////  These functions have been added exclusively for the AUTOv2
//////////////////////

export function _exactTokenInForLBPTOut(
    amount: OldBigNumber,
    poolPairData: PhantomStablePoolPairData
): OldBigNumber {
    // The formula below returns some dust (due to rounding errors) but when
...
/* 
Flow of calculations:
amountLBPTin -> newInvariant -> (amountOutProportional, amountOutBeforeFee) ->
amountOutPercentageExcess -> amountOut
*/
export function _exactLBPTInForTokenOut(
    amount: OldBigNumber,
    poolPairData: PhantomStablePoolPairData
): OldBigNumber {
    // The formula below returns some dust (due to rounding errors) but when
    ...
    **********************************************************************************************/
    ...
    const currentInvariant = _invariant(A, balances);
    // Calculate new invariant
    const bnumBalanceIn = bnum(formatFixed(poolPairData.virtualLbptSupply, 18));
...
export function _poolDerivativesLBPT(
    ...
    if (is_first_derivative) {
        ans = partial_x.div(minus_partial_D).times(lbptSupply).div(D);
    } else {
...
export function _spotPriceAfterSwapExactTokenInForLBPTOut(
    ...
    let bnumBalanceOut = bnum(formatFixed(virtualLbptSupply, decimalsOut));
    balances[tokenIndexIn] = balances[tokenIndexIn].plus(
        amount.times(feeFactor)
    );
    bnumBalanceOut = bnumBalanceOut.plus(
        _exactTokenInForLBPTOut(amount, poolPairData)
    );
...
export function _spotPriceAfterSwapTokenInForExactLBPTOut(
    ...
    const n = balances.length;
    const A = amp.div(n ** (n - 1));
    const _in = _tokenInForExactLBPTOut(amount, poolPairData);
    const feeFactor = _feeFactor(balances, tokenIndexIn, swapFee);
...
export function _spotPriceAfterSwapExactLBPTInForTokenOut(
    ...
    const _out = _exactLBPTInForTokenOut(amount, poolPairData);
    const feeFactor = _feeFactor(balances, tokenIndexOut, swapFee);
    let bnumBalanceIn = bnum(formatFixed(virtualLbptSupply, decimalsIn));
...
export function _spotPriceAfterSwapLBPTInForExactTokenOut(
    ...
    bnumBalanceIn = bnumBalanceIn.minus(
        _LBPTInForExactTokenOut(amount, poolPairData)
    );
    const ans = _poolDerivativesLBPT(
        A,
        balances,
...
export function _derivativeSpotPriceAfterSwapLBPTInForExactTokenOut(
    ...
    const _in = _LBPTInForExactTokenOut(amount, poolPairData);
    const feeFactor = _feeFactor(balances, tokenIndexOut, swapFee);
    balances[tokenIndexOut] = balances[tokenIndexOut].minus(
        amount.div(feeFactor)
    );
