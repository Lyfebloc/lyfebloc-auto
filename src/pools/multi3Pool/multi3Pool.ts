...
export type Multi3PoolPairData = PoolPairBase & {
    balanceTertiary: BigNumber; // Balance of the unchanged asset
    decimalsTertiary: number; // Decimals of the unchanged asset
};

export type Multi3PoolToken = Pick<
    SubgraphToken,
    'address' | 'balance' | 'decimals'
>;

export class Multi3Pool implements PoolBase<Multi3PoolPairData> {
    poolType: PoolTypes = PoolTypes.Multi3;
    id: string;
    address: string;
    tokensList: string[];
    tokens: Multi3PoolToken[];
    swapFee: BigNumber;
    ...
    static fromPool(pool: SubgraphPoolBase): Multi3Pool {
        ...
        if (pool.tokens.length !== 3)
            throw new Error('Multi3Pool must contain three tokens only');

        return new Multi3Pool(
            pool.id,
            pool.address,
            pool.swapFee,
            pool.totalShares,
            pool.tokens as Multi3PoolToken[],
            pool.tokensList,
            pool.root3Alpha
        );
    }
    ...
    parsePoolPairData(tokenIn: string, tokenOut: string): Multi3PoolPairData {
        const tI = Multi3Pool.findToken(
            this.tokens,
            tokenIn,
        ...
        const poolPairData: Multi3PoolPairData = {
            id: this.id,
    ...
    getNormalizedLiquidity(poolPairData: Multi3PoolPairData): OldBigNumber {
    ...
    getLimitAmountSwap(
        poolPairData: Multi3PoolPairData,
        swapType: SwapTypes
    ): OldBigNumber {
