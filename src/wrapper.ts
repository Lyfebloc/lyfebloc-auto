...
export class AUTO {
     ...
     * @param {AutoConfig} config - Chain specific configuration for the AUTO.
    ...
    constructor(
        ...
        private readonly config: AutoConfig,
    ...
    getPools(useLbpts?: boolean): SubgraphPoolBase[] {
        return this.poolCacher.getPools(useLbpts);
    }
     ...
     * @param swapOptions 
     * @param useLbpts Set to true to consider join/exit weighted pool paths (these will need formatted and submitted via Relayer)
     * @returns Swap information including return amount and swaps structure to be submitted to Reserve.
     */
    async getSwaps(
        tokenIn: string,
        tokenOut: string,
        ...
        if (this.useLbpt !== useLbpts) {
            options.forceRefresh = true;
            this.useLbpt = useLbpts;
        }
        const pools: SubgraphPoolBase[] = this.poolCacher.getPools(useLbpts);
