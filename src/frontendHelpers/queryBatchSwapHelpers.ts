 ...
 * approve them for the Reserve, or even know a user's address.
 */
export async function queryBatchSwap(
    ...
    return await reserveContract.queryBatchSwap(swapType, swaps, assets, funds);
}
...
*/
async function getAutoSwapInfo(
    tokenIn: string,
    tokenOut: string,
    swapType: SwapTypes,
    amount: string,
...
/*
Uses AUTO to create and query a batchSwap for multiple tokens in > single tokenOut.
For example can be used to join staLyfe3 with DAI/USDC/USDT.
*/
...
/*
Uses AUTO to create and query a batchSwap for a single token in > multiple tokens out.
For example can be used to exit staLyfe3 to DAI/USDC/USDT.
*/
