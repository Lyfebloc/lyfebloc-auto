...
describe('phantomStable pools tests', () => {
    ...
    context('phantomStable pools', () => {
        ...
        it('phantomStable LBPT -> token', () => {
            ...
            const amount = 4000;
            const outMath = phantomStableMath._exactLBPTInForTokenOut(
                bnum(amount),
                poolPairData
            );
            const outBigInt = stableMathBigInt._calcTokenOutGivenExactLbptIn(
            ...
            const inMath = phantomStableMath._LBPTInForExactTokenOut(
                bnum(amount),
                poolPairData
            );
            ...
            const inBigInt = stableMathBigInt._calcLbptInGivenExactTokensOut(
                poolPairData.amp.toBigInt(),
                poolPairData.allBalancesScaled.map((b) => b.toBigInt()),
                amountsOutBigInt,
                poolPairData.virtualLbptSupply.toBigInt(),
            ...
            const spPhantom =
                phantomStableMath._spotPriceAfterSwapExactLBPTInForTokenOut(
                    bnum(0),
                    poolPairData
                );
            ...
            const spBigInt =
                stableMathBigInt._spotPriceAfterSwapExactLBPTInForTokenOut(
                    poolPairData.amp.toBigInt(),
                    balances,
                    poolPairData.tokenIndexOut,
        ...
        it('phantomStable token -> LBPT', () => {
            ...
            // like in the previous case, LBPT -> token.
            const { a1, a2 } = getSwapOutcomes(
                phantomStablePool,
                poolPairData,
                4000
            ...
            const b1 = phantomStableMath._exactTokenInForLBPTOut(
                bnum(4000).times(priceRateIn),
                poolPairData
            );
            assert.approximately(
            ...
            const b2 = phantomStableMath
                ._tokenInForExactLBPTOut(bnum(4000), poolPairData)
                .div(priceRateIn);
            assert.approximately(
                a2.div(b2).toNumber(),
                1,
