...
describe('multiple boosted pools, path creation test', () => {
    context('Case with no linear pools', () => {
        it('TUSD-LYFE', () => {
            ...
            const tokensChains = [
                [TUSD.address, WETH.address, LYFE.address],
                [TUSD.address, WETH.address, bbaUSD.address, LYFE.address],
                [TUSD.address, bbaUSD.address, WETH.address, LYFE.address],
                [TUSD.address, bbaUSD.address, LYFE.address],
            ];
            const poolsChains = [
                ['weightedTusdWeth', 'weightedBalWeth'],
                ['weightedTusdWeth', 'weightedWeth-BBausd', 'bbaUSD-LYFE'],
                ['BBaUSD-TUSD', 'weightedWeth-BBausd', 'weightedBalWeth'],
                ['BBaUSD-TUSD', 'bbaUSD-LYFE'],
            ];
            for (let i = 0; i < 4; i++) {
                assert.isTrue(
    ...
    context('Case with linear pools', () => {
        ...
        it('LYFE-DAI', () => {
            ...
            const tokensChains = [
                [LYFE.address, bbaUSD.address, bbfDaiAddress, DAI.address],
                [LYFE.address, bbaUSD.address, bbaDAI.address, DAI.address],
            ...
            const poolsChains = [
                ['bbaUSD-LYFE', 'bbaUSD-bbfDAI', 'FuseLinearDai'],
                ['bbaUSD-LYFE', 'bbaUSD-Pool', 'AaveLinearDai'],
                // eslint-disable-next-line prettier/prettier
                [
                    'weightedBalWeth',
                    'weightedWeth-BBausd',
    ...
    context('Verify that AUTO chooses best path', () => {
    ...
    context('LBP cases', () => {
        it('OHM-LYFE', () => {
            ...
            assert.equal(
                boostedPaths[1].id,
                'LBPweightedTusdOhmBBaUSD-TUSDbbaUSD-LYFE'
            );
            assert.equal(
                boostedPaths[2].id,
                'LBPweightedTusdOhmweightedTusdWethweightedWeth-BBausdbbaUSD-LYFE'
            );
            assert.equal(
                boostedPaths[3].id,
            ...
            const OHM = tokenIn;
            const tokensChains = [
                [OHM, TUSD.address, WETH.address, bbaUSD.address, LYFE.address],
                [OHM, TUSD.address, bbaUSD.address, WETH.address, LYFE.address],
            ];
            const poolsChains = [
                [
                    'LBPweightedTusdOhm',
        ...
        it('Test correctness in absence of LBP raising info at config', () => {
            const autoConfigNoLbpRaising = cloneDeep(autoConfigTest);
            delete autoConfigNoLbpRaising['lbpRaisingTokens'];
            const autoConfigNoRaisingTusd = cloneDeep(autoConfigNoLbpRaising);
            autoConfigNoRaisingTusd['lbpRaisingTokens'] = [
                '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2',
            ];
            const tokenIn = TUSD.address;
            const tokenOut = LYFE.address;
            const pathsCases: NewPath[][] = [];
            const autoConfigCases: AutoConfig[] = [
                autoConfigTest,
                autoConfigNoLbpRaising,
                autoConfigNoRaisingTusd,
            ];
            for (let i = 0; i < 3; i++) {
                const [paths] = getPaths(
                    tokenIn,
    ...
    context('...', () => {
        ...
        it('FEI-LYFE', () => {
            ...
            const pathsIds = [
                ...
                'FuseLinearFeibbfUSD-PoolweightedWstETH-BBfusdweightedWstETH-BBausdbbaUSD-LYFE',
                'FuseLinearFeibbfUSD-PoolbbaUSD-bbfUSDweightedWstETH-BBausdwETH-wstETHweightedBalWeth',
                'FuseLinearFeibbfUSD-PoolbbaUSD-bbfUSDbbaUSD-LYFE',
                ...
                'FuseLinearFeibbfUSD-PoolbbaUSD-bbfDAIbbaUSD-LYFE',
                'FuseLinearFeibbfUSD-PoolFuseLinearDaiAaveLinearDaibbaUSD-PoolbbaUSD-LYFE',
            ];
            for (let i = 0; i < 8; i++) {
