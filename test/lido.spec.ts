...
import { autoConfigEth } from './lib/constants';
...
import { AUTO, SwapInfo, SwapTypes, SubgraphPoolBase } from '../src';
...
const poolStaBal = Lido.StaticPools.staLyfe[chainId];
...
describe(`Tests for Lido USD routes.`, () => {
    /*
    06/08/21 - 
    As initial roll out for Lido, until phantom LBPTs are ready, we are using single wstETH/WETH pool.
    Because current AUTO doesn't support more than one hop we hard code the following pairs/routes:
    ...
    */
    ...
    context('Handle stETH as input/output', async () => {
        ...
        it(`stETH swap should be same as wstETH with priceRate allowance, SwapExactIn, stETH In`, async () => {
            ...
            const swapInfostEth: SwapInfo = await auto.getSwaps(
                stETH,
                DAI,
                swapType,
                swapAmt,
            ...
            const swapInfowstEth: SwapInfo = await auto.getSwaps(
                wstETH,
                DAI,
                swapType,
