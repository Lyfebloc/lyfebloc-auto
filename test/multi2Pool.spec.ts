...
// TS_NODE_PROJECT='tsconfig.testing.json' npx mocha -r ts-node/register test/multi2Pool.spec.ts
import { expect } from 'chai';
import cloneDeep from 'lodash.clonedeep';
import { formatFixed, parseFixed } from '@ethersproject/bignumber';
import { JsonRpcProvider } from '@ethersproject/providers';
import { bnum } from '../src/utils/bignumber';
import { USDC, DAI, autoConfigEth } from './lib/constants';
import { SwapTypes, AUTO, SwapInfo, SubgraphPoolBase } from '../src';
// Add new PoolType
import { Multi2Pool } from '../src/pools/multi2Pool/multi2Pool';
// Add new pool test data in Subgraph Schema format
import testPools from './testData/multi2Pools/multi2TestPool.json';
import { MockPoolDataService } from './lib/mockPoolDataService';
import { mockTokenPriceService } from './lib/mockTokenPriceService';

describe('Multi2Pool tests USDC > DAI', () => {
    const testPool = cloneDeep(testPools).pools[0];
    const pool = Multi2Pool.fromPool(testPool);
