...
import { AUTO, SwapInfo, SwapTypes, PoolTypes, SubgraphPoolBase } from '../src';
import { bnum } from '../src/utils/bignumber';
import {
    MetaStablePool,
    MetaStablePoolPairData,
} from '../src/pools/metaStablePool/metaStablePool';
import { LYFE, autoConfigEth, USDC, WETH } from './lib/constants';
...
async function getStableComparrison(
    ...
    const autoStable = new AUTO(
        provider,
        autoConfigEth,
        new MockPoolDataService(stablePools),
        mockTokenPriceService
    );
    await autoStable.fetchPools();

    const swapInfoStable: SwapInfo = await autoStable.getSwaps(
        tokenIn,
        tokenOut,
        swapType,
        swapAmt,
...
describe(`Tests for MetaStable Pools.`, () => {
    ...
    context('multihop', () => {
        ...
        //     const auto = new AUTO(provider, gasPrice, maxPools, chainId, pools);

        //     const fetchSuccess = await auto.fetchPools([],false);
        //     expect(fetchSuccess).to.be.true;

        //     let swapInfo: SwapInfo = await auto.getSwaps(
        //         tokenIn,
        //         tokenOut,
        //         swapType,
        //         swapAmt
