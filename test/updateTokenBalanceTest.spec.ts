...
import { formatSwaps } from '../src/router/autoClass';
...
import { WETH, LYFE, LINEAR_AUSDC, LINEAR_ADAI } from './lib/constants';
...
describe('fails if token balances are not updated after a swap', () => {
    context('Weighted pool', () => {
        ...
        const path1 = createPath([WETH.address, LYFE.address], [pool]);
        const path2 = createPath([WETH.address, LYFE.address], [pool]);
        const path3 = createPath([WETH.address, LYFE.address], [pool]);
        assert.isNotTrue(path1 == path2);
        assert.isTrue(path1.pools[0] == path2.pools[0]);
        it('updateTokenBalance - WETH-LYFE', () => {
