...
describe(`Tests for PhantomStable Pools.`, () => {
    ...
    **/
        ...
        context('swapExactIn', () => {
            it(`TokenToToken should return valid limit`, async () => {
                testLimit(
                    ...
                    [poolsFromFile.STALYFE3[0] as SubgraphPoolBase],
                    0,
                    bnum('4755.30225811346514699')
                );
            });

            it(`TokenToLbpt should return valid limit`, async () => {
                testLimit(
                    LINEAR_ADAI.address,
                    bbaUSD.address,
                    SwapTypes.SwapExactIn,
            ...
            it(`LbptToToken should return valid limit`, async () => {
                testLimit(
                    bbaUSD.address,
    ...
    it('Test removeLBPT', () => {
        const metaStableLbptSwapPool = PhantomStablePool.fromPool(
            cloneDeep(pool)
        );
        // const poolPairData = metaStableLbptSwapPool.parsePoolPairData(
        const poolPairDataNoLBPT = metaStableLbptSwapPool.parsePoolPairData(
            MKR.address,
            stETH.address
        );

        expect(poolPairDataNoLBPT.tokenIndexIn).to.eq(2);
        expect(poolPairDataNoLBPT.tokenIndexOut).to.eq(1);
        expect(poolPairDataNoLBPT.allBalances).to.deep.eq([
            bnum(10),
            bnum(1000),
            bnum(300),
        ]);

        const balances = poolPairDataNoLBPT.allBalances;
        const expectedallBalancesScaled = balances.map((balance) =>
            parseFixed(balance.toString(), 18)
        );
        expect(poolPairDataNoLBPT.allBalancesScaled).to.deep.eq(
    ...
    it('Test removeLBPT', () => {
        ...
        expect(poolPairDataNoLBPT.tokenIndexIn).to.eq(2);
        expect(poolPairDataNoLBPT.tokenIndexOut).to.eq(0);
        expect(poolPairDataNoLBPT.allBalances).to.deep.eq([
            bnum(10),
            bnum(1000),
            bnum(300),
    ...
    context('Full Swaps', () => {
        let pool;
        beforeEach(() => {
            pool = cloneDeep(poolsFromFile.STALYFE3[0]) as SubgraphPoolBase;
        });
        context('SwapExactIn', () => {
            ...
            it('Token>LBPT, SwapExactIn', async () => {
                const returnAmount = await testFullSwap(
                    LINEAR_AUSDT.address,
                    bbaUSD.address,
                    SwapTypes.SwapExactIn,
            ...
            it('LBPT>Token, SwapExactIn', async () => {
                const returnAmount = await testFullSwap(
                    bbaUSD.address,
                    LINEAR_AUSDT.address,
                    SwapTypes.SwapExactIn,
        ...
        context('SwapExactOut', () => {
            ...
            it('Token>LBPT, SwapExactOut', async () => {
                const returnAmount = await testFullSwap(
                    LINEAR_AUSDT.address,
                    bbaUSD.address,
                    SwapTypes.SwapExactOut,
