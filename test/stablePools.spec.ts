...
import { LBPTForTokensZeroPriceImpact } from '../src/frontendHelpers/stableHelpers';
import { parseFixed } from '@ethersproject/bignumber';
import { LYFE, DAI, autoConfigEth, USDC, USDT } from './lib/constants';
...
describe(`Tests for Stable Pools.`, () => {
    ...
    context('direct pool', () => {
        it(`Full Swap - swapExactIn No Route`, async () => {
            ...
            const fetchSuccess = await auto.fetchPools();
    ...
    context('stable helpers', () => {
        it('should test LBPTForTokensZeroPriceImpact for 0.1% single token add', () => {
            const allBalances = [
                parseFixed('1000', 18),
                parseFixed('1000', 18),
                parseFixed('1000', 18),
            ...
            const lbptTotalSupply = parseFixed('3000', 18);
            const decimals = [18, 18, 18];

            const lbptAmt = LBPTForTokensZeroPriceImpact(
                allBalances,
                decimals,
            ...
            expect(lbptAmt.toString()).eq('1000000000000000000');
        });

        it('should test LBPTForTokensZeroPriceImpact for 1% single token add', () => {
            const allBalances = [
                parseFixed('1000', 18),
                parseFixed('1000', 18),
                parseFixed('1000', 18),
            ...
            expect(lbptAmt.toString()).eq('10000000000000000000');
        });

        it('should test LBPTForTokensZeroPriceImpact for 10% single token add', () => {
            const allBalances = [
                parseFixed('1000', 18),
                parseFixed('1000', 18),
                parseFixed('1000', 18),
            ...
            expect(lbptAmt.toString()).eq('100000000000000000000');
        });

        it('should test LBPTForTokensZeroPriceImpact for proportional add', () => {
            const allBalances = [
                parseFixed('1000', 18),
                parseFixed('1000', 18),
                parseFixed('1000', 18),
            ...
            expect(lbptAmt.toString()).eq('3000000000000000000');
        });

        it('should test LBPTForTokensZeroPriceImpact for single token add + uneven pool', () => {
            const allBalances = [
                parseFixed('2000', 18),
                parseFixed('1000', 18),
                parseFixed('1000', 18),
            ...
            const amp = parseFixed('500', 3);
            const amounts = [parseFixed('1', 18), Zero, Zero];
            const lbptTotalSupply = parseFixed('4000', 18);
            const decimals = [18, 18, 18];
            ...
            expect(lbptAmt.toString()).eq('999212124447530974');
        });

        it('should test LBPTForTokensZeroPriceImpact for single token add + VERY uneven pool', () => {
            const allBalances = [
                parseFixed('2000', 18),
                parseFixed('100', 18),
                parseFixed('100', 18),
            ...
            const amp = parseFixed('500', 3);
            const amounts = [parseFixed('1', 18), Zero, Zero];
            const lbptTotalSupply = parseFixed('2200', 18);
            const decimals = [18, 18, 18];
            ...
            expect(lbptAmt.toString()).eq('977051100758936668');
        });
        ...
        it('Derivative Bug Case', () => {
            ...
            const lbptTotalSupply = '2999263268368702307690295440';
            ...
            expect(lbptAmt.toString()).eq('28957866405645758917829');
        });
    });
});
