...
import poolsFromFile from './testData/stablePools/stablePoolWithLBPT.json';

describe('stable-math tests', () => {
    // Make a stable pool
    // const poolsFromFile: {
    //     pools: SubgraphPoolBase[];
    // } = require('./testData/stablePools/stablePoolWithLBPT.json');
    const pool = poolsFromFile.pools[0] as SubgraphPoolBase;
    const stableLbptSwapPool = StablePool.fromPool(pool);
    ...
    const amp = bnum(stableLbptSwapPool.amp.toString());
    const error = 0.00001;
