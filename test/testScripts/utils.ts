...
import { FundManagement, AUTO, SwapInfo, SwapTypes } from '../../src';
import { reserveAddr } from './constants';

import erc20abi from '../abi/ERC20.json';

// Helper to check/set allowances for tokens being traded via reserve
export async function handleAllowances(
    ...
    if (tokenIn !== AddressZero) {
        // Reserve needs approval for swapping non ETH
        console.log('Checking reserve allowance...');
        ...
        if (allowance.lt(amount)) {
            console.log(
                `Not Enough Allowance: ${allowance.toString()}. Approving reserve now...`
            );
            const txApprove = await tokenInContract
                .connect(wallet)
                .approve(reserveAddr, MaxUint256);
...
export async function printOutput(
    ...
    // This calculates the cost to make a swap which is used as an input to auto to allow it to make gas efficient recommendations.
    // Note - tokenOut for SwapExactIn, tokenIn for SwapExactOut
    const outputToken = swapType === SwapTypes.SwapExactIn ? tokenOut : tokenIn;
    const cost = await auto.getCostOfSwapInToken(
        outputToken.address,
