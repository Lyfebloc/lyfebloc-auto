// TS_NODE_PROJECT='tsconfig.testing.json' npx mocha -r ts-node/register test/multi3Pool.spec.ts
...
// Add new PoolType
import { Multi3Pool } from '../src/pools/multi3Pool/multi3Pool';
// Add new pool test data in Subgraph Schema format
import testPools from './testData/multi3Pools/multi3TestPool.json';

describe('Multi3Pool tests USDC > DAI', () => {
    const testPool: SubgraphPoolBase = cloneDeep(testPools).pools[0];
    const pool = Multi3Pool.fromPool(testPool);
