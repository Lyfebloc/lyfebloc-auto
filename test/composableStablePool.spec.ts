...
describe('composable stable pool', () => {
    ...
    context('out given in, several consistencies', () => {
        ...
        it('token -> LBPT spot price with no rate', () => {
            // spot prices
            poolPairData.swapFee = BigNumber.from(0);
            const spPhantom =
                phantomStableMath._spotPriceAfterSwapExactTokenInForLBPTOut(
                    bnum(0),
                    poolPairData
                );
            ...
            const spBigInt =
                stableMathBigInt._spotPriceAfterSwapExactTokenInForLBPTOut(
        ...
        it('composableStablePool token -> LBPT', () => {
            const lbptOut = composableStablePool._exactTokenInForTokenOut(
                poolPairData,
                bnum(1)
            );
            const poolSP =
