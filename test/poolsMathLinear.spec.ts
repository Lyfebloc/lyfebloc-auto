...
describe('poolsMathLinear', function () {
    ...
    describe('init', () => {
        ...
        const lbptSupply = s(0);

        it('given main in within lower and upper', async () => {
            const mainIn = s(5);
            const result = linear._calcLbptOutPerMainIn(
                mainIn,
                mainBalance,
                wrappedBalance,
                lbptSupply,
    ...
    context('with main below lower', () => {
        ...
        const lbptSupply = s(49.5);

        context('swap lbpt & main', () => {
            context('main in', () => {
                ...
                it('given LBPT out', async () => {
                    const lbptOut = s(100.65);
                    const result = linear._calcMainInPerLbptOut(
                       lbptOut,
                        mainBalance,
                        wrappedBalance,
                        lbptSupply,
            ...
            context('main out', () => {
                it('given LBPT in', async () => {
                    const bptIn = s(10.1);
                    const result = linear._calcMainOutPerLbptIn(
                        bptIn,
                        mainBalance,
                        wrappedBalance,
                        lbptSupply,
                ...
                it('given main out', async () => {
                    const mainOut = s(10);
                    const result = linear._calcLbptInPerMainOut(
                        mainOut,
                        mainBalance,
                        wrappedBalance,
                        lbptSupply,
        ...
        describe('swap lbpt & wrapped', () => {
            it('given wrapped in', async () => {
                const wrappedIn = s(5);
                const result = linear._calcLbptOutPerWrappedIn(
                    wrappedIn,
                    mainBalance,
                    wrappedBalance,
                    lbptSupply,
            ...
            it('given LBPT out', async () => {
                const lbptOut = s(5);
                const result = linear._calcWrappedInPerLbptOut(
                   lbptOut,
                    mainBalance,
                    wrappedBalance,
                    lbptSupply,
            ...
            it('given LBPT in', async () => {
                const bptIn = s(5);
                const result = linear._calcWrappedOutPerLbptIn(
                    bptIn,
                    mainBalance,
                    wrappedBalance,
                    lbptSupply,
            ...
            it('given wrapped out', async () => {
                ...
                const result = linear._calcLbptInPerWrappedOut(
                    wrappedOut,
                    mainBalance,
                    wrappedBalance,
                    lbptSupply,
    ...
    describe('spot prices', () => {
        ...
        it('LbptOutPerMainIn', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    checkDerivative(
                        linear._calcLbptOutPerMainIn,
                        linear._spotPriceAfterSwapLbptOutPerMainIn,
                        amount,
                        mainBalance,
                        8000, // wrappedBalance
                        3500, // lbptSupply
        ...
        it('MainOutPerLbptIn', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    checkDerivative(
                        linear._calcMainOutPerLbptIn,
                        linear._spotPriceAfterSwapMainOutPerLbptIn,
                        amount,
                        mainBalance,
                        100, // wrappedBalance
                        3500, // lbptSupply
        ...
        it('MainInPerLbptOut', () => {
            // missing test case: lbptSupply = 0
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    for (const lbptSupply of [3500]) {
                        checkDerivative(
                            linear._calcMainInPerLbptOut,
                            linear._spotPriceAfterSwapMainInPerLbptOut,
                            amount,
                            mainBalance,
                            8000, // wrappedBalance
                            lbptSupply,
        ...
        it('LbptInPerMainOut', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    ...
                    checkDerivative(
                        linear._calcLbptInPerMainOut,
                        linear._spotPriceAfterSwapLbptInPerMainOut,
                        amount,
                        mainBalance,
                        8000, // wrappedBalance
                        3500, // lbptSupply
        ...
        // lbpt<>wrapped
        it('WrappedOutPerLbptIn', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    checkDerivative(
                        linear._calcWrappedOutPerLbptIn,
                        linear._spotPriceAfterSwapWrappedOutPerLbptIn,
                        amount,
                        mainBalance,
                        8000, // wrappedBalance
                        3500, // lbptSupply
        ...
        it('LbptOutPerWrappedIn', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    checkDerivative(
                        linear._calcLbptOutPerWrappedIn,
                        linear._spotPriceAfterSwapLbptOutPerWrappedIn,
                        amount,
                        mainBalance,
                        8000, // wrappedBalance
                        3500, // lbptSupply
        ...
        it('WrappedInPerLbptOut', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    for (const lbptSupply of [0, 3500]) {
                        checkDerivative(
                            linear._calcWrappedInPerLbptOut,
                            linear._spotPriceAfterSwapWrappedInPerLbptOut,
                            amount,
                            mainBalance,
                            8000, // wrappedBalance
                            lbptSupply,
        ...
        it('LbptInPerWrappedOut', () => {
            for (const mainBalance of mainBalanceTest) {
                for (const amount of [50, 70, 150, 300, 700, 1000, 2000]) {
                    checkDerivative(
                        linear._calcLbptInPerWrappedOut,
                        linear._spotPriceAfterSwapLbptInPerWrappedOut,
                        amount,
                        mainBalance,
                        8000, // wrappedBalance
                        3500, // lbptSupply
...
function checkDerivative(
    ...
    const val2 = fn(amount, mainBalance, wrappedBalance, lbptSupply, params);
