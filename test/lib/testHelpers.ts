...
import { autoConfigTest } from './constants';
    ...
    for (const pool of allPools) {
        ...
        if (pool.tokens.length != 0) {
            if (pool.tokens[0].balance != '0') {
                ...
                if (tokens.length > 1) {
                    allTokens.push(tokens.auto()); // Will add without duplicate
                }
...
export function displayResults(
    ...
    const tableData: {
        AUTO: string;
        'Full AUTO Time': number;
        'Return Amt': string;
    }[] = [];
    Results.forEach((result) => {
        tableData.push({
            AUTO: result.title,
            'Full AUTO Time': result.timeData.fullSwap,
...
export async function getFullSwap(
    ...
    if (swapType === 'swapExactIn' || swapType === SwapTypes.SwapExactIn)
        await auto.swapCostCalculator.setNativeAssetPriceInToken(
            tokenOut,
