...
import reserveAbi from '../../src/abi/Reserve.json';
...
export async function getOnChainBalances(
    ...
    subgraphPoolsOriginal.forEach((pool) => {
        ...
        multiPool.call(`${pool.id}.poolTokens`, reserveAddress, 'getPoolTokens', [
            pool.id,
        ]);
        ...
        } else if (pool.poolType.toString().includes('Multi')) {
            multiPool.call(
                `${pool.id}.swapFee`,
                pool.address,
                'getSwapFeePercentage'
...
/*
PoolDataService to fetch onchain balances of Multi3 pool.
