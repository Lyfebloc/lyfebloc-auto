...
import { DAI, autoConfigEth, USDC, WETH } from './lib/constants';
import {
    MockPoolDataService,
    mockPoolDataService,
...
describe(`Tests for wrapper class.`, () => {
    ...
    it(`Should return no swaps when pools not retrieved.`, async () => {
        ...
        const swaps: SwapInfo = await auto.getSwaps(
            tokenIn,
            tokenOut,
            swapType,
            swapAmt,
    ...
    it(`should have a valid swap`, async () => {
        ...
        const result: boolean = await auto.fetchPools();
    ...
    //     const auto = new AUTO(provider, chainId, null, pools);

    //     const result: boolean = await auto.fetchPools([], false);
    //     assert.isTrue(result);
    ...
    it(`compare weth/eth swaps, SwapExactIn, Weth In`, async () => {
        ...
        let result: boolean = await auto.fetchPools();
        assert.isTrue(result);

        const swapInfoEth: SwapInfo = await auto.getSwaps(
            tokenInEth,
            tokenOut,
            swapType,
            swapAmt,
